...
{
    public class SendZeroCountUseCaseTest
    {
        [Fact]
        public void ExecuteShouldWriteResponseFile()
        {
            ...
            ISendCountZeroUseCase useCase = new SendCountZeroUseCase(sendResponseUseCase.Object);

            useCase.Execute(expected);

            response.Should().NotBeNull();
            response.Info.Should().Be("Count zero: info");
            response.Quantity.Should().Be(2);
            response.Source.Should().Be("source");
            response.Target.Should().Be("target");
            response.Status.Should().Be(AutomationStatus.CountIsZero);
