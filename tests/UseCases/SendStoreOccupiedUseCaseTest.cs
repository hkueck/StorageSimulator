...
{
    public class SendStoreOccupiedUseCaseTest
    {
        [Fact]
        public void ExecuteShouldWriteResponseFile()
        {
            ...
            ISendStoreOccupiedUseCase useCase = new SendStoreOccupiedUseCase(sendResponseUseCase.Object);

            useCase.Execute(expected);

            response.Should().NotBeNull();
            response.Info.Should().Be("Store occupied: info");
            response.Quantity.Should().Be(2);
            response.Source.Should().Be("source");
            response.Target.Should().Be("target");
            response.Status.Should().Be(AutomationStatus.ShippedNotAllItems);
