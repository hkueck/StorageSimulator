...
{
    public class SendOrderExistUseCaseTest
    {
        [Fact]
        public void ExecuteShouldWriteResponseFile()
        {
            ...
            ISendOrderExistsUseCase useCase = new SendOrderExistsUseCase(sendResponseUseCase.Object);

            useCase.Execute(expected);

            response.Should().NotBeNull();
            response.Info.Should().Be("Order already exists: info");
            response.Quantity.Should().Be(2);
            response.Source.Should().Be("source");
            response.Target.Should().Be("target");
            response.Status.Should().Be(AutomationStatus.OrderAlreadyExists);
