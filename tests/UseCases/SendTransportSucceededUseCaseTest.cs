...
{
    public class SendTransportSucceededUseCaseTest
    {
        [Fact]
        public void ExecuteShouldWriteResponseFile()
        {
            ...
            ISendTransportSucceededUseCase useCase = new SenTransportSucceededUseCase(sendResponseUseCase.Object);

            useCase.Execute(expected);

            response.Should().NotBeNull();
            response.Info.Should().Be("Transport: info");
